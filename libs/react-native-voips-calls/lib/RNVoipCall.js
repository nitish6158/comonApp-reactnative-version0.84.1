...
class RNVoipCallNativeModule {
    ...
    initializeCall = async (options) => new Promise((resolve, reject) => {
        if (!RNVoipCall) {
            reject({ error: true, message: 'RNVoipCall native module is not linked' });
            return;
        }
        if (!options.appName) {
    ...
    onRemoveEndCallListener = () => {
    this.removeEventListener("endCall");
  };
    ...
    rejectCall = (uuid) => {
      ...
      endCall = (uuid) => !RNVoipCall ? null : IsIOS ?  RNVoipCall.endCall(uuid) : RNVoipCall.clearNotificationById(uuid);
      ...
      stopRingtune = () => !IsIOS && RNVoipCall ? RNVoipCall.stopRingtune() : null;
      
      //Missed Call
      showMissedCallNotification = (title, body, uuid) => RNVoipCall ? RNVoipCall.showMissedCallNotification(title,body,uuid) : null;
